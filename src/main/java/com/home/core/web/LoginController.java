...
public class LoginController {
	...
	public String get3drSessionKey(@RequestParam("code") String code,@RequestParam("name") String name
			,@RequestParam("img") String img,@RequestParam("geom") String geom)
			throws Exception {
		System.out.println(code);
		if (code != null && code.length() > 0&&name != null && name.length() > 0
				&&img != null && img.length() > 0&&geom != null && geom.length() > 0) {
			return loginService.get3drSessionKey(code,name,img,geom);
		} else {
			return "";
		}
	}
